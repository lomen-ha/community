import { LatestBoardData } from 'data/boardData';
import Link from 'next/link';

export default function Latest() {
  const { latestBoardData } = LatestBoardData();

  return (
    <div className="flex flex-col">
      <h1 className="text-center text-xl font-semibold">최신</h1>
      <table className="mt-4 border-collapse border-t border-b border-slate-400">
        <tbody>
          {latestBoardData?.detail?.map((item) => (
            <>
              <tr key={item.id}>
