import { ArtistBoardData } from 'data/boardData';
import Link from 'next/link';

export default function Artist() {
  const { artistBoardData } = ArtistBoardData();

  return (
    <div className="flex flex-col">
      <h1 className="text-center text-xl font-semibold">아티스트</h1>
      <table className="mt-4 border-collapse border-t border-b border-slate-400">
        <tbody>
          {artistBoardData?.detail?.map((item) => (
            <>
              <tr key={item.id}>
