import { GoodsBoardData } from 'data/boardData';
import Link from 'next/link';

export default function Goods() {
  const { goodsBoardData } = GoodsBoardData();

  return (
    <div className="flex flex-col">
      <h1 className="text-center text-xl font-semibold">굿즈</h1>
      <table className="mt-4 border-collapse border-t border-b border-slate-400">
        <tbody>
          {goodsBoardData?.detail?.map((item) => (
            <>
              <tr key={item.id}>
